...
export const financials = pgTable("financials", {
  id: serial("id").primaryKey(),
  reportId: integer("report_id").references(() => reports.id, { onDelete: "cascade" }).notNull(),
  originalContractAmt: numeric("original_contract_amt", { precision: 12, scale: 2 }).notNull(),
  paidToDate: numeric("paid_to_date", { precision: 12, scale: 2 }).notNull(),
});
...
export const reportSnapshots = pgTable("report_snapshots", {
  ...
  reportId: integer("report_id")
    ...
    .notNull(),
  snapshotData: text("snapshot_data").notNull(), // store JSON as text
...
// Inverse relations (Child → Report)
